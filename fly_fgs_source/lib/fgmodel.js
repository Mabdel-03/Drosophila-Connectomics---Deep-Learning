...
(function (root) {
  ...
  function init(bundle, opts) {
    ...
    const st = {
      ...
      scene: { figW: opts.figW ?? 16, figH: opts.figH ?? 120, figOn: opts.figOn !== false, scale: opts.scale ?? 10, startAz: opts.startAz ?? 0, seedFig: opts.seedFig ?? 23, seedGnd: opts.seedGnd ?? 17, contrast: opts.contrast ?? 1, groundOn: opts.groundOn !== false,
        gratingOn: opts.gratingOn ?? false, gratingPeriod: opts.gratingPeriod ?? 30, gratingHi: opts.gratingHi ?? 0.75, gratingLo: opts.gratingLo ?? 0,
        paperMode: opts.paperMode === true, paperScene: opts.paperScene || null },   // rotating optomotor drum or paper random-dot scene
  ...
  function lumSnap(st, az, el, snapIdx) {
    ...
    if (S.paperMode) {
      const paper = root.PaperFGS || (typeof require === "function" ? require("./paper_stimulus.js") : null);
      if (!paper) throw new Error("PaperFGS is required for paper-mode luminance");
      if (!S.paperScene) S.paperScene = paper.createScene();
      return paper.luminanceAt(S.paperScene, az, el, {
        figure_angle_deg: r.dF[snapIdx],
        ground_angle_deg: r.dG[snapIdx],
        figure_mean_azimuth_deg: S.startAz,
        figure_width_deg: S.figW,
  ...
  // Paper assay: body/head are fixed and both layer positions are supplied
  // explicitly.  The legacy integrator remains untouched; pre-positioning the
  // ground state makes its post-step snapshot equal the requested angle.
  function stepPaper(st, ctrl) {
    if (!st.scene.paperMode) throw new Error("stepPaper requires paperMode=true");
    const figureAngle = Number(ctrl.figureAngleDeg);
    const groundAngle = Number(ctrl.groundAngleDeg);
    const figureVelocity = Number(ctrl.figureVelocityDegS);
    const groundVelocity = Number(ctrl.groundVelocityDegS);
    if (![figureAngle, groundAngle, figureVelocity, groundVelocity].every(Number.isFinite)) {
      throw new TypeError("paper controls must contain finite explicit angles and velocities");
    }
    st.groundWorldDisp = groundAngle - groundVelocity * st.dt;
    step(st, {
      heading: 0,
      headingVel: 0,
      figWorldAz: figureAngle,
      figVelWorld: figureVelocity,
      groundVelWorld: groundVelocity,
    });
    // Guard against accumulated floating-point integration error: downstream
    // snapshots and replay metadata always expose the prescribed position.
    st.groundWorldDisp = groundAngle;
    st.dG = groundAngle;
  }
  ...
  const API = { init, step, stepPaper, readout, lumNow, hinesPrepare, hinesSolve, _internal: { lumSnap, filtLum, computeDrive } };
